...
public class ChunksController : MonoBehaviour
{
    ...
    async void GenerarChunkAsync(Vector2Int coord)
    {
        if (!juegoActivo) return;

        // Preparar datos en hilo secundario (sin tocar Unity API)
        ...
        // Volver al hilo principal: instanciar GameObject y mallas
        ...
        chunk.RegenerateMesh(bloquesDisponibles, GetBlockAtWorldPos);
    }
    ...
    void RemoveBlock(Vector3Int pos)
    {
        ...
        if (EsBordeDelChunk(localPos))
        {
            ...
            foreach (var v in vecinos)
            {
                ...
                if (chV != null) chV.RegenerateMesh(bloquesDisponibles, GetBlockAtWorldPos);
            }
        }
    ...
    public int GetBlockAtWorldPos(Vector3Int pos)
    {
        ...
        if (!chunksInfinitos.TryGetValue(coord, out var chunk)) return 0;
